$(function() {
	...
	$(document).on("keypress keyup", "#jumlah", function(e){
		...
		if (jumlah > stok) {
			$("#jumlah").val(1)
			toastr.error(`Jumlah melebihi stok, maksimal ${stok}`, 'Informasi', {timeOut: 500});
		}
	...
	function modal_edit_item() {
		...
		if (parseInt(jumlah) > parseInt(stok)) {
			toastr.error("Jumlah melebihi stok, maksimal " + stok, '', {timeOut: 500});
			$("#item_jumlah").val(1);
						...
						success: function (response) {
							if (response.status) {
								Swal.fire({
									...
									window.open(`${BASE_URL}/penjualan/cetak/${response.idpenjualan}`)
